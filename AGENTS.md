...
This is the single source of truth for coding agents working in this repository.
`CLAUDE.md` is a three-line pointer that imports this file, so Claude Code
discovery keeps working — put content here, never there.
...
- `.gitignore` already excludes `CLAUDE.local.md`, `*.local.*` (which also covers
  `AGENTS.local.md`), `tfplan`, `.terraform/`
