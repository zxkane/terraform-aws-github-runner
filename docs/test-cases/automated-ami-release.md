...
| TC-AMI-108 | Validate amd64 browsers | Playwright Chromium starts; the Chrome D-Bus wrapper and exact runner `CHROME_PATH` are present; Google Chrome starts through that path as the `ubuntu` runner user with a temporary profile, browser sandboxing enabled and a 90-second hard timeout |
