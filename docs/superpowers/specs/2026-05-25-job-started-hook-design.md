...
- **Skip every `_`-prefixed entry at the outer level.** The runner agent prefixes ALL of its own `_work` helper dirs with `_` (`_actions`, `_temp`, `_tool`, `_PipelineMapping`, `_update`, `_diag`, …) precisely so they never collide with per-repo workdirs (a repo workdir is named after the repo and never starts with `_`). The original v2 hook enumerated a fixed four-entry list and missed `_update` — the runner's self-update staging dir. Its `externals/node20/…` residue can't always be unlinked by `ubuntu`, so `find -delete` errored and (under `set -e`) aborted the hook, failing healthy jobs before any step ran (observed downstream 2026-06-15). Matching `_*` instead of a fixed list closes that class of bug.
...
- **`set -euo pipefail`, but the per-workdir wipe is best-effort.** The hook still uses strict mode, but the actual `find -delete` is wrapped (`… 2>/dev/null; then … else warn`) so housekeeping can NEVER fail the job. Rationale revised from v2 ("fail loudly"): the dominant field failure was the hook killing *healthy* jobs, not poison leaking through. Even when an entry inside a real repo workdir can't be unlinked (e.g. a root-owned file from a Docker step), `find` still removes everything it can — including the corrupted `.git/index` at the workdir root — so `actions/checkout` re-clones successfully. A stranded sub-entry is strictly better than a failed job; the warning goes to stderr to keep the signal loud without the job-kill.
